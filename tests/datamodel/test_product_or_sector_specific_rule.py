...
def test_valid_product_or_sector_specific_rule():
    ...
    assert product_or_sector_specific_rule.rule_names.non_empty_strings[1] == "rule 2"


def test_invalid_product_or_sector_specific_rule_other_operator_name_undefined():
    # Invalid input: other_operator_name must be undefined when operator is not Other
    operator = ProductOrSectorSpecificRuleOperator.PEF
    ...
    with pytest.raises(
        ValueError,
        match="other_operator_name must be undefined when operator is not Other",
    ):
        ProductOrSectorSpecificRule(
            operator=operator,
